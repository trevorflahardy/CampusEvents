      ...
      {event.bannerUrl ? (
          ...
          <div className="absolute bottom-0 left-0 w-full p-6 md:p-10 max-w-6xl mx-auto inset-x-0">
            <div className="flex flex-wrap items-center gap-2 mb-4">
              {event.categories.map((cat) => (
                <span key={cat.id} className="px-3 py-1 rounded-md bg-white/15 backdrop-blur-sm text-white/90 text-xs font-bold uppercase tracking-wider border border-white/10">
                  {cat.name}
                </span>
              ))}
              <span className={`badge uppercase tracking-wide ${statusColors[event.status] || "badge-neutral"}`}>
                {event.status}
              </span>
            </div>
            <h1 className="text-3xl md:text-5xl font-bold text-white leading-tight mb-4 tracking-tight drop-shadow-lg">
              <EditableField value={event.title} fieldName="title" eventId={event.id} canEdit={canEdit} onSaved={fetchEvent} className="text-3xl md:text-5xl font-bold text-white leading-tight tracking-tight" inputClassName="text-xl font-bold" />
            </h1>
        ...
        <section className="relative w-full overflow-hidden">
          ...
          <div className="max-w-6xl mx-auto px-4 md:px-8 pt-2 pb-6">
            <div className="flex flex-wrap items-center gap-2 mb-4">
              {event.categories.map((cat) => (
                <span key={cat.id} className="rounded-md bg-brand-glow text-[#1a4f3b] border border-brand-light px-3 py-1 text-xs font-bold uppercase tracking-wider">
                  {cat.name}
                </span>
              ))}
              <span className={`badge uppercase tracking-wide ${statusColors[event.status] || "badge-neutral"}`}>
            ...
            <h1 className="text-3xl md:text-5xl font-bold text-slate-800 leading-tight mb-4 tracking-tight">
              <EditableField value={event.title} fieldName="title" eventId={event.id} canEdit={canEdit} onSaved={fetchEvent} className="text-3xl md:text-5xl font-bold text-slate-800 leading-tight tracking-tight" inputClassName="text-xl font-bold" />
            </h1>
            <div className="flex flex-wrap items-center gap-5 text-slate-500 text-sm font-medium">
              <div className="flex items-center gap-2">
                <svg className="w-5 h-5 text-[#1a4f3b]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
                <span>{formatDate(event.startTime)}</span>
              </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="glass-heavy rounded-2xl p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-5 animate-fade-in stagger-1">
              ...
              <div className="shrink-0 flex items-center gap-3">
                ...
                {hasRegistered && user?.role === "student" && event.status !== "cancelled" && (
                  <button
                    disabled
