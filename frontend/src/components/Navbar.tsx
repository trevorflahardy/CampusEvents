...
/** App-wide navigation: desktop sidebar + mobile top bar + mobile bottom pill nav. */
export default function Navbar() {
  ...
  const mobileLinkClasses = (path: string) =>
    `flex flex-col items-center cursor-pointer transition-colors ${
      isActive(path)
        ? "text-[#1a4f3b] dark:text-emerald-400"
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex glass w-64 h-screen flex-col pt-8 pb-6 px-4 fixed left-0 top-0 z-40 border-r border-slate-200/60 dark:border-white/20">
        ...
        <Link
          to="/"
          className="flex items-center gap-2 cursor-pointer px-2 mb-2"
        >
        ...
        {isAuthenticated && user ? (
          <div className="flex items-center gap-3 mb-8 px-2 mt-3">
            ...
            <div className="min-w-0">
              <h2 className="font-semibold text-sm leading-tight truncate dark:text-white">
                {user.name}
              </h2>
              <p className="text-xs text-slate-500 dark:text-[#94a3b8] capitalize">
                {user.role}
              </p>
            </div>
          </div>
