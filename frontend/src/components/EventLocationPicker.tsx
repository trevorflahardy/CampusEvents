    ...
    <div className="space-y-2">
      ...
      <div ref={wrapperRef} className="relative">
        ...
        {showSuggestions && suggestions.length > 0 && (
          <ul className="absolute z-50 w-full mt-1 glass-heavy rounded-xl overflow-hidden shadow-lg border border-white/20 dark:border-white/10">
            {suggestions.map((s) => (
              <li key={s.id}>
                <button
                  type="button"
                  onClick={() => handleSelect(s)}
                  className="w-full text-left px-4 py-2.5 text-sm text-slate-700 dark:text-slate-200 hover:bg-[#1a4f3b]/8 dark:hover:bg-emerald-400/10 transition-colors cursor-pointer flex items-start gap-2"
                >
